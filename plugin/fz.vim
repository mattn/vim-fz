...
command! -nargs=* -complete=dir FzMRU call fz#run({'basepath': <q-args>, 'type': 'list', 'list': map(filter(map(copy(v:oldfiles), 'expand(v:val)'), 'filereadable(v:val)'), 'fnamemodify(v:val, ":.:~:gs!\\!/!")')})
